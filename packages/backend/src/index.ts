...
const config = {
  ...
  frontend: process.env.FRONTEND_URI
    ? process.env.FRONTEND_URI + "/Dashboard"
...
const start = async () => {
  try {
    ...
    await system.createActor(SessionStore, {
      name: "SessionStore",
      strategy: "Restart",
    });
    await system.createActor(FingerprintStore, {
      name: "FingerprintStore",
      strategy: "Restart",
    });
    await system.createActor(UserStore, {
      name: "UserStore",
      strategy: "Restart",
    });
    await system.createActor(QuizActor, {
      name: "QuizActor",
      strategy: "Restart",
    });
    await system.createActor(ErrorActor, {
      name: "ErrorActor",
      strategy: "Restart",
      errorReceiver: true,
    });
