...
export class QuestionActor extends SubscribableActor<Question, QuestionActorMessage, ResultType> {
	...
	constructor(name: string, system: ActorSystem, private uid: Id) {
			...
			return await QuestionActorMessages.match<Promise<ResultType>>(message, {
				...
				Update: async question => {
					...
					return existingQuestion
						.map(async c => {
							if (!["SYSTEM", "TEACHER", "ADMIN"].includes(clientUserRole) && clientUserId !== c.authorId) {
				...
				Delete: async id => {
					const existingQuestion = await this.getEntity(id);

					return await existingQuestion
						.map(async q => {
							if (!["SYSTEM", "TEACHER", "ADMIN"].includes(clientUserRole) && clientUserId !== q.authorId) {
								return new Error("Invalid write access to question");
							}
